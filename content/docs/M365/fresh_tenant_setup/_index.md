...
Follow the current recommendations [here](https://learn.microsoft.com/en-us/entra/identity/role-based-access-control/security-emergency-access) to set up two EEAs. 
...
- Disable adding GA to local admin.  
    ![Screenshot of Microsoft Entra join and registration settings webpage with various toggles and options.](entra_device_settings.png)
...
1. Create the app registration.  
   ![Screenshot of a registration form for an application, with fields for name, account type selection, and redirect URI.](entra_app_reg_ps_sdk_1.png)
...
- Enable Password writeback in On-premises integration  
    ![Screenshot of password management settings in a software interface.](entra_password_writeback.png)
...
- Passkeys
- Authenticator  
    ![Screenshot of a software configuration page for Microsoft Authenticator options settings.](authentication_methods_authenticator_configure.png)
...
![](entra_auth_method_migration_complete.png)
...
- Disable verification options in the [legacy MFA settings portal](https://account.activedirectory.windowsazure.com/usermanagement/mfasettings.aspx)  
    ![Interface showing user authentication options: phone call, SMS, app notification, and code verification.](entra_legacy_mfa.png)
- Disable Authentication methods in [SSPR Authentication Methods](https://entra.microsoft.com/#view/Microsoft_AAD_IAM/PasswordResetMenuBlade/~/AuthenticationMethods/fromNav/Identity)  
    ![User authentication setup interface with options for mobile app, email, and phone methods.](entra_sspr_auth_methods.png)
- Migrate to the [Converged Authentication Methods Policy](https://entra.microsoft.com/#view/Microsoft_AAD_IAM/AuthenticationMethodsMenuBlade/~/AdminAuthMethods)  
    ![Manage migration dialog box with options for pre-migration, in progress, and complete.](entra_auth_meothd_migration.png)
...
- Toggle On
  ...
  - Restrict access to Entra ID administration portal  
    ![Administrative interface settings for user role permissions, guest access, and linked account options.](entraid_user_settings.png)
...
:link: [Portal](https://entra.microsoft.com/#view/Microsoft_AAD_IAM/CompanyRelationshipsMenuBlade/~/Settings)

- Change guest invite settings to `Member users and users assigned to specific admin roles can invite guest users including guests with member permissions`  
![](entra_guest_invite_settings.png)
...
- Enable LAPS  
    ![Option to enable Microsoft Entra LAPS with 'Yes' and 'No' buttons, 'Yes' highlighted.](entra_device_enable_laps.png)
...
- Enable Enterprise State Roaming  
    ![Enterprise State Roaming settings page on a Microsoft Entra ID interface, with options to save or discard changes.](entra_devices_enable_dsr.png)
...
- Enable the policy, targeting all users and excluding the group you just created.  
    ![Multifactor authentication registration policy interface with user inclusion/exclusion controls and policy enforcement toggle.](entra_identity_protection_mfa_policy.png)
...
- Enable all diagnostic settings to log to your Sentinel log analytics workspace  
    ![Interface of Azure diagnostic setting for log categories and destination details.](entra_diag_setings.png)
...
![Web interface showing steps for activating Global Secure Access with icons, buttons, and links.](entra_gsa_enable.png)

#### Internet Access

1. Enable the [Microsoft Profile](https://entra.microsoft.com/#view/Microsoft_Azure_Network_Access/ForwardingProfile.ReactView)  
    ![Interface with traffic profiles for Microsoft and Internet access, including policy and user assignment options.](entra_gsa_enable_ms_profile.png)
...
![UI settings for Windows data and license verification with toggles set to 'On'.](intune_windows_data_connector.png)
...
![undefined](intune_mde_connector2.png)
...
- Run the prereq check. You'll see an advisory for co-management, this can be safely disregarded.![](intune_autopatch_prereq_check.png)
- Grant admin access for Microsoft![](intune_autopatch_admin_access.png)
...
- Put a couple of devices in the Test ring by clicking on the device name, then selecting Device Actions -> Assign Ring. In the flyout, choose the Test ring![](intune_windows_autopatch_device_ring.png)
...
![intune_windows_m365_apps](intune_windows_m365_apps.png)
![intune_windows_m365_apps_1](intune_windows_m365_apps_1.png)
...
- Set MDM and MAM user scopes to all
  ![intune_auto_enrollment](intune_auto_enrollment.png)
...
![](intune_autopilot_device_prep.png)
...
![](intune_apple_enrollment_profile.png)
...
2. Export the profile you just created. Copy the URL.  
    ![](intune_apple_config_export_profile.png)
...
!["Screenshot of Managed Google Play enrollment page."](intune_android_linking.png)
...
- Create a MacOS enrollment profile [here](https://intune.microsoft.com/#view/Microsoft_Intune_Enrollment/UserInitiatedEnrollmentProfilesListBlade) if you didn't for iOS yet - they're shared between iOS and MacOS.  
    [](intune_macos_enrollment_profile.png)
- Download the Company Portal app for MacOS from [here](https://go.microsoft.com/fwlink/?linkid=853070) and deploy the company portal app as a MacOS LOB app  
    ![](intune_macos_company_portal.png)
...
![](intune_macos_platform_sso-2.png)
...
- Endpoint Detection and Response
  - Create a new EDR policy targeting Windows. Target all devices.  
    ![](intune_edr_profile-1.png)
    ![intune_edr_profile_1](intune_edr_profile_1.png)
- Antivirus
  - Create a new Microsoft Defender Antivirus profile  
    ![](intune_mde_windows_av_policy.png)
  - Enable Network Protection in Block mode. Target all devices.  
    ![](intune_mde_windows_av_policy_1.png)
...
- Deploy the combined profile
  - Create a device configuration profile for macOS devices using a custom template  
    ![](intune_macos_mde_device_config.png)
...
- Create a Windows LAPS profile and apply to all devices.  
  ![intune_laps_profile](intune_laps_profile.png)

  ![intune_laps_profile1](intune_laps_profile1.png)
...
- Enable unified SIEM and XDR.  
  ![](defender_unified_xdr_1.png)

### Email & collaboration

- Peset Security Configuration Policies
  - Enable Standard Protection Preset Policies.  
    ![mdo_protection_policies](mdo_protection_policies.png)

    ![mdo_protection_policies_exo](mdo_protection_policies_exo.png)

    ![mdo_protection_policies_m365](mdo_protection_policies_m365.png)
...
- System
  - IP Address Ranges
    ...
    ![mdca_ip_address_range](mdca_ip_address_range.png)
- Cloud Discovery
  - Defender for Endpoint
    - Enforce App Access: `On`
  - User Enrichment
    - Enrich discovered identities: `On` 
  ...
  - Configure Group Managed Service Account account
    - On the first DC
      ...
      - On the other DCs, purge kerberos tickets and install the service account
        ...
        ![Adding a gMSA account](mdi_gmsa_account.png)
...
Rationale and planning guidance are [here](https://learn.microsoft.com/en-us/purview/deploymentmodels/depmod-securebydefault-intro) . I've changed some of the configurations to more closely match what I've seen done with customers.
...
Content marking: 
- Footer: `Classified as Confidental`
Auto-labeling: `Off`
Groups & Sites Protection Settings:
...
Default Labels:
- Documents: `Confidential/All Employees`
- Emails: `General`
	- Inherit label from attachments: `Automatically apply`


##### Additional Controls
- Consider setting SharePoint files to be [sensitive by default](https://learn.microsoft.com/en-us/sharepoint/sensitive-by-default). Guest access to newly added content is restricted until it undergoes a sensitive content scan and applicable DLP policies with content-based conditions are enforced. 
- Use labels to [set the default sharing link](https://learn.microsoft.com/en-us/purview/sensitivity-labels-default-sharing-link) for SharePoint sites to `People in your organization`.


### DLP

#### Settings
[:link: Portal](https://compliance.microsoft.com/datalossprevention)
...
[:blue_book: Docs](https://learn.microsoft.com/en-us/purview/endpoint-dlp-learn-about#just-in-time-protection)
...
1. Create a Data Leaks policy from the template
   ![](purview_irm_policies_data_leaks_1.png)
2. Target all users
3. Choose not to prioritize content.
   ![](purview_irm_policies_data_leaks_2.png.png)
...
:link:[Portal](https://purview.microsoft.com/purviewforai/unifiedoverview) :blue_book:[Docs](https://learn.microsoft.com/en-us/purview/ai-microsoft-purview)
...
- Enable the [SharePoint Entra B2B Integration](https://learn.microsoft.com/en-us/sharepoint/sharepoint-azureb2b-integration)
...
- Change Sharing settings from the insecure defaults
	- Move the external sharing sliders to Content can be shared with: `New and existing guests`
	- Uncheck `Allow guests to share items they don't own`
- Change File and folder default links to: `Only people in your organization`
![](sp_sharing_settings.png)
